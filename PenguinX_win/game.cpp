...
Uint8* keys;
//******************
short int dy_bg=2,a_bg=0;
short int x_t=200,y_t=400,dy_t=4,dx_t=4;
...
int main(int argc, char *argv[])
{
  ...
  SDL_WM_SetCaption ("PenguinX", NULL);
  TTF_Init();
...
void DrawTux()
{
  ...
  {
    SDL_Surface *temp;
    temp=ZoomIMG(tux.SGetFrameIMG(0),zoom_tux,0);
    ...
    if (ttime-time_zoom>10)
    {
      ...
      if (temp) zoom_tux+=.009;
      if (temp==false) zoom_tux-=.009;
    }
...
void TestTime()
{
  ...
  if (ttime-time_aspeed>1000){dy_bg=2;}
}
      ...
      if (obj[i].live==true and obj[i].img<=3)
      {
        ...
        if ((tux.SPCollide(obj_img.SGetFrameIMG(obj[i].img-1),obj[i].x,obj[i].y)))
        {
          ...
          if (obj[i].img==3)
          {
            ...
            time_jump=SDL_GetTicks();
            time_aspeed=SDL_GetTicks();dy_bg=3;
          }
